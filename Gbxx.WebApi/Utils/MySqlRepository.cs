...
CASE
	...
	c.info Contents,
	'' Curl,
	concat( 'https://pic.2v7qe.cn', REPLACE(b.thumbpic,'/pic/','/') ) ImageThums,
    ...
    CAST(
    DATE_FORMAT(
    from_unixtime( unix_timestamp( '2020-09-01 00:00:00' ) + 
                   floor( rand() * ( unix_timestamp( '{0}' ) - unix_timestamp( '2020-09-01 00:00:00' ) + 1 ) ) ),'%Y-%m-%d %H:%i:%s' ) AS datetime) PushTime, 
    b.`from` Source,	
...
WHERE
    ...
    AND b.id < {1} ORDER BY b.id DESC LIMIT {2},{3}; COMMIT; ",
